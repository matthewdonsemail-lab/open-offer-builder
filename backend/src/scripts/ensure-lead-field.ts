...
async function main() {
  ...
  const obj = data.objects.edges.map((e:any)=>e.node).find((o:any)=>o.nameSingular==='agencyLead');
  if (!obj) { console.error('agencyLead not found'); process.exit(1); }
  console.log(`Found agencyLead ${obj.id} fields:`, obj.fieldsList.map((f:any)=>`${f.name}:${f.type}`).join(', '));
  const has = obj.fieldsList.some((f:any)=>f.name==='qualificationStatus');
  if (has) { console.log('✓ qualificationStatus already exists', obj.fieldsList.find((f:any)=>f.name==='qualificationStatus')); return; }
  console.log('Creating qualificationStatus SELECT field...');
  const res: any = await gql(`mutation CreateOneFieldMetadataItem($input: CreateOneFieldMetadataInput!) { createOneField(input: $input) { id name type } }`, { input: { field: { objectMetadataId: obj.id, isActive: true, isNullable: true, isUnique: false, name: 'qualificationStatus', label: 'Qualification Status', type: 'SELECT', description: 'Qualified or Disqualified from quiz', options: [{ label: 'Qualified', value: 'QUALIFIED', color: 'green', position: 0 }, { label: 'Disqualified', value: 'DISQUALIFIED', color: 'red', position: 1 }] } } });
  ...
  const vobj = verify.objects.edges.map((e:any)=>e.node).find((o:any)=>o.nameSingular==='agencyLead');
