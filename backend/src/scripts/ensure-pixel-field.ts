...
if(obj.fieldsList.some((f:any)=>f.name==='metaPixelId')){console.log('exists');process.exit(0)}
const res:any=await gql(`mutation CreateOneFieldMetadataItem($input: CreateOneFieldMetadataInput!) { createOneField(input: $input) { id name type } }`,{input:{field:{objectMetadataId:obj.id,isActive:true,isNullable:true,isUnique:false,name:'metaPixelId',label:'Meta Pixel ID',type:'TEXT',description:'Facebook Meta Pixel ID for this offer (fires PageView + Lead where fbLead true)'}}});
