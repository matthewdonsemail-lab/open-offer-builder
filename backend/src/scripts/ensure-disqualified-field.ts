...
if(obj.fieldsList.some((f:any)=>f.name==='disqualifiedConfig')){console.log('exists');process.exit(0)}
const res:any=await gql(`mutation CreateOneFieldMetadataItem($input: CreateOneFieldMetadataInput!) { createOneField(input: $input) { id name type } }`,{input:{field:{objectMetadataId:obj.id,isActive:true,isNullable:true,isUnique:false,name:'disqualifiedConfig',label:'Disqualified Config',type:'RAW_JSON',description:'Disqualified page blocks JSON'}}});
