...
async function main() {
  ...
  const has = obj.fieldsList.some((f:any)=>f.name==='thankYouConfig');
  if (has) { console.log('✓ thankYouConfig already exists'); return; }
  console.log('Creating thankYouConfig RAW_JSON field...');
  const res: any = await gql(`mutation CreateOneFieldMetadataItem($input: CreateOneFieldMetadataInput!) { createOneField(input: $input) { id name type } }`, { input: { field: { objectMetadataId: obj.id, isActive: true, isNullable: true, isUnique: false, name: 'thankYouConfig', label: 'Thank You Config', type: 'RAW_JSON', description: 'Thank-you page blocks JSON' } } });
